...
# Here aggr graph using mean-main option
PARTITION=${1}
NUM_SENTS=${2}
BATCH_SIZE=${3}
NUM_GPUS=${4}
...
WORD_VOCABS="words-mix"
WORD_SIZES="5000 10000 25000"
DEFAULT_PARAMS="--num_sents ${NUM_SENTS} --batch_size ${BATCH_SIZE} --grid_search --model_type graph --model gcn --task ranking --grid_search --aggr mean-main"
...
for main_vocab in ${BPE_VOCABS}; do
    exp_name="ranking_gcn_mean-main_main_vocab_${main_vocab}_side_vocabs_None_word_vocab_size_None_neg_samples_${neg_sample}"
...
for main_vocab in ${WORD_VOCABS}; do
    for word_size in ${WORD_SIZES}; do
        exp_name="ranking_gcn_mean-main_main_vocab_${main_vocab}_side_vocabs_None_word_vocab_size_${word_size}_neg_samples_${neg_sample}"
...
for main_vocab in ${WORD_VOCABS}; do
    for side_vocab in ${BPE_VOCABS}; do
        for word_size in ${WORD_SIZES}; do
            exp_name="ranking_gcn_mean-main_main_vocab_${main_vocab}_side_vocabs_${side_vocab}_word_vocab_size_${word_size}_neg_samples_${neg_sample}"
